...
function print_organizations($parent_id,
	...
	if ( is_array($top_level) ) {
		...
		foreach ($top_level as $garbage => $content) {
			...
			$content['text'] = format_content($content['text'], $content['formatting'], $glossary, $path, $current_forum);
...
function print_resources_forum() {
    ...
    //ovviamente la destinazione dovrà essere calcolata!!!NON PUò ESSERE STATICA!
    /* TODO ******************************* aggiunta immagine all'archivio*/
