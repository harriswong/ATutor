...
if ($_POST['cancel_pass']) {
	header('Location: ' . $_SERVER['PHP_SELF']);
}

/**
 * New password submitted, verify
 */
if ($_POST['acc_pass']) {
	// password check
	if ($_POST['password'] == '') { 
	...
	if (!$msg->containsErrors()) {
		...
		$sha1_pass;
		if (strlen($_POST['password_hidden']) < 40) { // <noscript> as client end
			$sha1_pass = sha1($_POST['password']);
		} else {
			$sha1_pass = $_POST['password_hidden'];
		}
		
		$sql = "UPDATE ".TABLE_PREFIX."members SET password='$sha1_pass' WHERE member_id=$_SESSION[member_id]";
...
<script language="JavaScript">
  function crypt_sha1() {
  	document.acc_form.password_hidden.value = hex_sha1(document.reg_form.password.value);
  	document.form.form_password.value = "";
  	return true;
  }
</script>
<noscript>
<?php $msg->printWarnings('SHA1_NONE'); ?>
</noscript>

<form name="reg_form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
...
<tr>
	<?php
	 if (!isset($_POST['chg_pass'])) { 
			echo '<td class="row1" align="center" valign="top" colspan="2">' .
					'<br/><form method="post" action="' . $_SERVER['PHP_SELF'] .'">' .
					'<input type="submit" class="button" value="Change your Password" name="chg_pass">' .
					'</form>' .
				'</td>' .
				'</tr><tr><td class="row1" align="center" valign="top" colspan="2"><br/></td>';
	 } else { // form fields to add new password 
	
		echo '<td class="row1" align="right" valign="top"><br/><label for="password">'. _AT('new_password') . ':</label></td>' .
			'<td class="row1" valign="top"><br/><input id="password" class="formfield" name="password" type="password"  size="15" maxlength="15" value="" /><br />' .
				'<small class="spacer">&middot;' . _AT('combination') . '<br />' .
				'&middot;' . _AT('15_max_chars') . '</small></td>' .
			'</tr>' .
			'<tr>' .
				'<td class="row1" align="right"><label for="password2">' . _AT('password_again') . ':</label></td>' .
				'<td class="row1"><input id="password2" class="formfield" name="password2" type="password" size="15" maxlength="15" value="" /></td>' .
			'</tr>' .
			'<tr>' .
				'<td class="row1" align="center" valign="top" colspan="2">' .
					'<form name="acc_form" method="post" action="' . $_SERVER['PHP_SELF'] .'">' .
					'<input type="hidden" name="password_hidden">' .
					'<input type="submit" class="button" onClick="return crypt_sha1();" value="Accept Change" name="acc_pass">&nbsp;' .
					'<input type="submit" class="button" value="Cancel" name="cancel_pass">' .
					'</form>' . 
				'</td>' . 
				'</tr><tr><td class="row1" align="center" valign="top" colspan="2"><br/></td>';
	}
