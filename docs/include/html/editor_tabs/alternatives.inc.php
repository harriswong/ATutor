		...
		if ($n==0)
			...
			$sql	= "SELECT * FROM ".TABLE_PREFIX."primary_resources WHERE content_id=".$cid." order by primary_resource_id";
	      	...
	      	while ($row = mysql_fetch_assoc($result)) {
	      		...
	      		for ($i=0; $i < $n; $i++){
	      			if (trim($resources[$i])==trim($row['resource'])) {
	      				...
	      				<div class="resource_box">
							...
							<?php checkbox_types($row[primary_resource_id], 'primary');
	      				...
	      				if (mysql_num_rows($result_alt) > 0) {
							...
							while ($alternative = mysql_fetch_assoc($result_alt)){
								...
								<div class="alternative_box">
		      						...
		      						<?php 
		      						checkbox_types($alternative['secondary_resource_id'], 'secondary');
		      						$languages = $languageManager->getAvailableLanguages();
				...
				if ($present==false){
					$sql_sel 	= "SELECT primary_resource_id FROM ".TABLE_PREFIX."primary_resources WHERE content_id=".$cid." and resource='".$row['resource']."'";
					$result_sel = mysql_query($sql_sel, $db);
					$id = mysql_fetch_assoc($result_sel);
										
					$sql_del 	= "DELETE FROM ".TABLE_PREFIX."primary_resources WHERE primary_resource_id=".$id."'";
...
function on_load()
	{
		if (document.getElementById("single_resources").checked){
  		...
  		if (document.form.alternatives.value==1){
			document.getElementById('2').style.display = "none";
     		document.getElementById('1').style.display = "block";
    		}
    	else
